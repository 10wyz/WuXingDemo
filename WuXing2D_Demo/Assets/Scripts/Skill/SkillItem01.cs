...
namespace WuXingSkill
{
    ...
    /// 技能组件：破坏目标卡牌
    /// </summary>
    public class SkillItem01 : SkillItem
    {
        private TargetItem m_targetCard;
        private List<GameObject> m_selectCard;//选择卡牌
        private bool m_attackFinish;//是否完成攻击

        public SkillItem01()
        {
            m_skillItemName = "SkillItem01";
        ...
        public override bool LaunchSkill(GameObject origin, SkillControl skillControl, bool free)
        {
            ...
            while(m_selectCard.Count > 0)
            {
                ...
                {
                    m_attackFinish = true;
                    if (!free && !skillControl.AddWaitEvent(origin, origin, target, TriggerType.Attack))
                        return false;
                }
                else
                {
                    ...
                    if (GameListener.Instance().ChainEventNotify(ChainEventID.evt_destory, eventPara))
                    {
                        if (!free && !skillControl.AddWaitEvent(origin, origin, target, TriggerType.Damage))
                            return false;
                    }
                }
            }
