...
namespace WuXingSkill
{
    ...
    /// 技能组件：获取卡组外卡牌
    /// </summary>
    public class SkillItem11 : SkillItem
    {
        ...
        public override bool LaunchSkill(GameObject origin, SkillControl skillControl, bool free)
        {
            ...
            while (m_selectCard.Count > 0)
            {
                ...
                if (GameListener.Instance().ChainEventNotify(ChainEventID.evt_tohand, eventPara))
                {
                    if (!free && !skillControl.AddWaitEvent(origin, origin, target, TriggerType.ToHand))
                        return false;
                }
            }
